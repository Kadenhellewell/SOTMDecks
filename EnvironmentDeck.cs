...
namespace SOTMDecks
{
    internal class EnvironmentDeck : CardCollection
    {
        public EnvironmentDeck(string fileName) : base("Deck")
        {
        ...
        private static List<EnvironmentCard> ParseDeck(JObject json)
        {
        ...
        public Option<EnvironmentCard> Draw(bool fromBottom = false)
        {
            if (cards_.Count == 0)
            {
                Console.WriteLine("No cards to draw");
                return Option.None<EnvironmentCard>();
            }
            EnvironmentCard drawn;
            if (fromBottom)
            {
        ...
        public Option<List<EnvironmentCard>> GetTopCards(int n)
        {
            if (cards_.Count < n)
            {
                Console.WriteLine($"Fewer than {n} cards");
                return Option.None<List<EnvironmentCard>>();
            }
